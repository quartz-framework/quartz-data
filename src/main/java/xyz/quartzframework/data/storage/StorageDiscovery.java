...
public class StorageDiscovery {
    ...
    public StorageDiscovery addExplicitStorages(Collection<Class<?>> storages) {
        this.explicitStorages.addAll(storages);
        return this;
    }
    ...
    public Set<Class<?>> discover() {
        Set<Class<?>> storages = new HashSet<>(explicitStorages);
        ...
        for (Object discoverer : discoverers.values()) {
            ...
            if (config != null) {
                storages.addAll(Arrays.asList(config.value()));
                Collections.addAll(basePackages, config.basePackages());
            }
        }

        for (String pkg : basePackages) {
            storages.addAll(ClassUtil.scan(
                new String[]{pkg},
                quartzApplication.exclude(),
                c -> c.isInterface() && c.isAnnotationPresent(Storage.class),
